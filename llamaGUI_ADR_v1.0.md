...
# DD-013 — 系統工程缺口修正總結（v2.5.x / v2.6.x）

決策ID：DD-013
標題：API 契約與 Engine 運作制度缺口修正 — 批次核可記錄
狀態：Accepted
日期：2026-05-12
優先級：✅ 已完成
關聯：DD-001 ~ DD-012（全系列）
版本快照：engine.py v2.5.2 / api.py v2.6.2

---

## 背景

本決策作為 DD-001~012 系列的封存節點。
在完成 DD-001~011 的功能落地後，針對 v2.5/v2.6 進行了一次完整系統工程審查，
識別出六類缺口（BUG #1~#8），並分兩輪修正與決策。
本文件為該輪所有核可項目的集中記錄。

---

## 第一輪：Bug 修正（engine.py v2.5.1 / api.py v2.6.1）

### BUG-FIX #1 — generate() 雙重佇列計數
- 檔案：engine.py
- 問題：`LlamaEngine.generate()` 呼叫 `self.stream()`，
  而 `stream()` 內含 `_check_queue()` guard，
  導致同一請求佔用兩個 waiting slot，MAX_WAITING=5 實際效果減半。
- 修正：`generate()` 改為直接呼叫 `self.active.stream()`，
  queue guard 僅在 `generate()` 一層執行。
- 狀態：✅ 已落地

### BUG-FIX #3 — SSE 串流錯誤格式不符業界標準
- 檔案：api.py
- 問題：engine 錯誤（`[Error: ...]`）夾在 `delta.content` 裡透傳，
  外部 SDK（openai-python、LangChain）會將其視為正常模型輸出，
  無法在客戶端層正確捕捉。
- 修正：錯誤時改發符合 OpenAI 格式的 error event：
  `{"error": {"message": ..., "type": "engine_error", "code": 500}}`
  再送 `data: [DONE]`，不污染 `delta.content`。
- 狀態：✅ 已落地

### BUG-FIX #6 — crash 路徑未清 LlamaEngine._current_profile
- 檔案：engine.py
- 問題：`_idle_monitor` 偵測到 subprocess crash 後，
  呼叫 `self.active._stop_server()`（清空 SubprocessEngine 狀態），
  但 `LlamaEngine._current_profile` 未同步清空，造成狀態殘留。
  後續 `GET /stats` 或 `POST /load` 可能讀到已失效的 profile 資料。
- 修正：crash 分支補加 `self._current_profile = {}`。
- 狀態：✅ 已落地

### BUG-FIX #8 — _download_tasks dict 永不清理
- 檔案：api.py
- 問題：每次 `POST /hf-download` 產生一筆 task，完成後永不移除，
  長時間運行下持續累積記憶體。
- 修正：
  - 新增 `_TASK_TTL_SEC = 3600`
  - 新增 `_prune_download_tasks()`，清除超過 TTL 的 done/error/cancelled task
  - `_run()` 完成時記錄 `finished_at`
  - `GET /hf-download/{task_id}` 每次呼叫前機會性清理
  - `DELETE /hf-download/{task_id}` 取消時也設定 `finished_at`
- 狀態：✅ 已落地

---

## 第二輪：DD-012 設計項目（engine.py v2.5.2 / api.py v2.6.2）

### DD-012 #4 — Profile CRUD 無 Schema 防護
- 檔案：api.py
- 問題：`POST /profiles` 的 `ProfileSaveRequest.profile` 型別為裸 `dict[str, Any]`，
  任何非法欄位或錯誤型別（如 `n_ctx: "abc"`）均可寫入 profiles.json，
  錯誤只在 engine 載入時才浮現。
- 修正：
  - 新增 `ProfileData` Pydantic model，涵蓋所有已知欄位並附型別宣告
  - `model_config = {"extra": "allow"}` 保留自訂欄位向下相容
  - `ProfileSaveRequest.profile` 型別改為 `ProfileData`
  - `save_profile_api()` 呼叫 `req.profile.model_dump()` 轉換後存入
  - 已知欄位型別錯誤於 FastAPI 422 層攔截，不寫入磁碟
- 狀態：✅ 已落地

### DD-012 #5 — 載入中間狀態不可觀測 + 並發載入無保護
- 檔案：engine.py + api.py
- 問題：
  1. `POST /load` 同步阻塞（最長 60s），期間 `GET /health` 的 `model_loaded`
     持續為 `false`，外部 polling client 無法區分「正在載入」與「未載入」。
  2. 並發兩個 `POST /load` 會導致兩個執行緒同時進入 `load()`，
     行為未定義（可能雙重啟動 subprocess 或雙重 `_unload_internal`）。
- 修正：
  - `LlamaEngine.__init__` 新增 `_is_loading: bool` + `_loading_lock: threading.Lock`
  - `load()` 重構為 guard wrapper + `_load_internal()`：
    進入時 `_is_loading = True`，finally 保證歸 False
  - 並發第二個 `load()` 直接回傳 `(False, "model is currently loading...")`
  - 新增 `is_loading` property 對外暴露
  - `GET /health` 新增 `model_loading` 欄位（bool）
  - `POST /load` 在 `is_loading=True` 時回 `409 Conflict`
    `{"detail": "model is currently loading — try again shortly"}`
- 狀態：✅ 已落地

### DD-012 #2 — TOCTOU 競態（文件說明，不修改程式）
- 問題：`ensure_model_loaded()` 與後續 `stream()` 之間存在微小視窗，
  若 idle auto-unload 在此期間觸發，`stream()` 開始時模型已不在，
  客戶端收到 SSE error event（BUG-FIX #3 已正確格式化）。
- 決策：不加鎖（加鎖會使 `_switch_lock` 覆蓋整個推論時間，
  造成嚴重 contention）。建議文件補充：
  > "If idle_unload_seconds < 60, a rare race may produce a single 503/error SSE.
  >  Retry once. For production use, set idle_unload_seconds ≥ 300."
- 狀態：✅ 已決策（文件補充，不修改程式）

### DD-012 #7 — 跨層命名無型別驗證（技術債）
- 問題：profile dict key（`n_gpu_layers`、`top_p` 等）在
  HTTP body → profile dict → engine kwarg 三層間靠 `.get()` fallback，
  拼字錯誤會靜默使用預設值。
- 決策：推遲至 v4.0 engine 重構，以 `TypedDict` 或 dataclass 統一。
  現有 `.get()` 模式在所有已測路徑均有合理 default，無已知靜默失敗案例。
- 狀態：✅ 已決策（列為 tech-debt，v4.0 處理）
...
| 版本 | 主要變更 |
|---|---|
| engine.py v2.3 | 原始版本（DD-001~006 前） |
| engine.py v2.4 | DD-001~006 落地 |
| engine.py v2.5 | DD-007~011 落地（標準參數、Tool Calling、KV Slot、VLM） |
| engine.py v2.5.1 | BUG-FIX #1、#6 |
| engine.py v2.5.2 | DD-012 #5（_is_loading guard） |
| api.py v2.4 | 原始版本 |
| api.py v2.5 | DD-007~011 落地 |
| api.py v2.6 | DD-011 VLM ContentPart、/v1/completions |
| api.py v2.6.1 | BUG-FIX #3、#8 |
| api.py v2.6.2 | DD-012 #4（ProfileData）、#5（409、model_loading） |
...
| 項目 | 描述 | 目標版本 |
|---|---|---|
| DD-012 #7 | profile dict 跨層命名無型別驗證 | v4.0 engine 重構 |
| DD-012 #2 | TOCTOU 競態（文件說明已補，程式不改） | 持續觀察 |
| UI-001 | 圖片拖放上傳 → base64 → ContentPartImage 組裝 UI | 後續 UI tab 決策 |
...
決策ID：DD-012
標題：API 契約完整性補強 — 錯誤語意、狀態可觀測性、Schema 防護
狀態：Accepted
日期：2026-05-12
優先級：🟡 高
來源分析：v2.5/v2.6 系統工程缺口審查（BUG #2、#4、#5、#7）

### 情境

承接 v2.5.1 / v2.6.1 的 bug 修正，本決策處理四個剩餘的設計缺口：
這些問題不屬於程式崩潰等級，但會在整合測試、外部客戶端對接、長時間運行時造成
行為不一致或可維護性下降。
...
| 編號 | 類型 | 描述 |
|---|---|---|
| #2 | 競態（可接受） | `ensure_model_loaded()` 與 `stream()` 之間的 TOCTOU 視窗 |
| #4 | 防護缺失 | `POST /profiles` 接受裸 dict，profile 結構無 schema 驗證 |
| #5 | 可觀測性 | `POST /load` 同步阻塞，`GET /health` 在載入期間無 `"loading"` 中間狀態 |
| #7 | 技術債 | profile dict key 在 HTTP body → profile dict → engine kwarg 三層間無型別驗證 |

### 決策

**#2 — TOCTOU：僅補文件說明，不修改程式**

`ensure_model_loaded()` 在 `_switch_lock` 保護下完成，但後續的 `stream()` 不在同一鎖的保護範圍。
這個競態視窗（idle unload 或 `POST /unload` 在兩者之間介入）會使 `stream()` 回傳
`[Model not loaded]`，api.py 已在 streaming 路徑中處理此錯誤（BUG-FIX #3 發 error event）。
不修改：加鎖範圍會導致 `_switch_lock` 覆蓋整個推論時間，反而造成更長的 lock contention。

文件補充（`llm_full.md` 的 Constraints 區段）：
> "If idle-unload is enabled with a very short timeout (< 60s), a rare race condition may
> cause a `503` response immediately after auto-loading. Retry once if this occurs."

**#4 — Profile Schema 防護：新增 `ProfileRequest` Pydantic model**

`POST /profiles` 的 `ProfileSaveRequest.profile: dict[str, Any]` 改為引用
`ProfileData` Pydantic model，允許已知欄位的型別驗證，額外欄位以 `model_config =
ConfigDict(extra="allow")` 容許（避免破壞現有 profile 的自訂欄位）。
...
`ProfileSaveRequest.profile` 型別改為 `ProfileData`。核心欄位型別錯誤（如
`n_ctx: "abc"`）在 FastAPI 422 層即攔截，不寫入 profiles.json。

**#5 — 載入中間狀態：新增 `loading` 狀態旗標**

`LlamaEngine` 新增 `_is_loading: bool = False`，在 `load()` 進入時設為 True，
結束時（成功或失敗）設回 False，以 `threading.Lock` 保護。

`GET /health` 回傳新增 `model_loading: bool` 欄位：
```json
{
  "status": "ok",
  "model_loaded": false,
...
```
`POST /load` 若在載入進行中被重複呼叫，回傳 `409 {"detail": "model is currently loading"}` 而非允許並發載入。

**#7 — 跨層命名技術債：記錄，不立即修正**

profile dict key（如 `n_gpu_layers`、`top_p`）與 Pydantic field（`ChatRequest.top_p`）
目前靠 `.get("top_p", default)` fallback 隱藏拼字錯誤。
本決策不修改，原因：
- 引入強型別 profile dataclass 需要全部 engine 呼叫點同步修改，風險大
- 現有 `.get()` 模式在所有已測路徑中均有合理 default，無已知靜默失敗案例

列為 tech-debt，待 v4.0 engine 重構時以 `TypedDict` 或 dataclass 統一。

### 實作順序

1. `ProfileData` Pydantic model + `ProfileSaveRequest` 修改（api.py，低風險）
2. `LlamaEngine._is_loading` + `GET /health` + `POST /load` 409 保護（engine.py + api.py，中風險）
3. `llm_full.md` 補充 TOCTOU 說明與 `has_vision` / `model_loading` 欄位文件

### 後果

- ✅ Profile 寫入有 schema 防護，不合法型別 422 攔截
- ✅ `GET /health` 可正確反映載入中狀態，外部 polling client 不再誤判
- ✅ 並發 `POST /load` 呼叫有明確的 409 語意，而非 undefined behavior
- ⚠️ `ProfileData(extra="allow")` 仍允許任意欄位寫入，自訂欄位不受型別保護
- ❌ #7（跨層命名）推遲至 v4.0，屬已知技術債
